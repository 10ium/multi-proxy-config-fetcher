...
"""
    ...
    def save_channel_stats(self, source_channels: List['ChannelConfig'], total_unique_configs_overall: int):
        ...
        for channel in source_channels:
            stats_data["channels"].append({
                ...
                # این مقادیر نیاز به ردیابی دقیق‌تری دارند اگر بخواهیم
                # "valid_configs_processed" و "unique_configs_found" را به طور دقیق برای هر کانال داشته باشیم.
                # در حال حاضر، این آمارها در سطح کلی قابل اعتمادتر هستند تا در سطح هر کانال.
                # برای این پروژه، این فیلدها در ChannelMetrics به عنوان هدف برای آینده باقی می‌مانند.
    ...
    def _generate_protocol_distribution_chart(self, protocol_counts: Dict[str, int]) -> str:
        ...
        if not protocols or sum(counts) == 0:
        ...
        # مرتب‌سازی برای نمایش بهتر در نمودار
        sorted_data = sorted(protocol_counts.items(), key=lambda item: item[1], reverse=True)
        sorted_protocols = [item[0].replace('://', '') for item in sorted_data] # حذف '://' برای نمایش تمیزتر
