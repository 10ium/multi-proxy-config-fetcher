...
class ConfigFetcher:
    ...
    def _fetch_raw_data_for_channel(self, channel: ChannelConfig) -> Tuple[List[str], List[str], Dict[str, Any]]:
        ...
        channel_status_info: Dict[str, Any] = {
            ...
            'success': False,
            'response_time': 0,
            'valid_configs_count': 0, # این در فاز 3 پر می‌شود
            'total_configs_raw': 0,
            'error_message': None # برای ذخیره پیام خطا در صورت واکشی ناموفق
        }
                ...
                if self.validator.validate_protocol_config(clean_config, actual_protocol):
                    ...
                    with self._lock: 
                        if canonical_id not in self.seen_configs:
                            ...
                            # **تغییر یافته**: لاگ‌های پردازش تک کانفیگ به DEBUG
                            ...
                            # **تغییر یافته**: لاگ کانفیگ تکراری به DEBUG
    ...
    def run_full_pipeline(self):
        """
        **تغییر یافته**: متد اصلی برای اجرای کامل pipeline واکشی، پردازش، توازن و ذخیره کانفیگ‌ها.
        """
        ...
        # **تغییر یافته**: کشف کانال‌ها از تمامی کانفیگ‌های خام جمع‌آوری شده
        ...
        # **تغییر یافته**: اضافه کردن تمامی URLهای جدید کشف شده به لیست منابع اصلی
                ...
                if self.validator.validate_protocol_config(clean_config, actual_protocol):
                    ...
                    with self._lock: 
                        if canonical_id not in self.seen_configs:
                            ...
                            self.seen_configs.add(canonical_id) 
                            # **تغییر یافته**: protocol_counts در اینجا به روز رسانی می شود (در _process_single_raw_config)
                            self.protocol_counts[actual_protocol] = self.protocol_counts.get(actual_protocol, 0) + 1 
...
def save_configs(configs: List[Dict[str, str]], config: ProxyConfig):
    ...
    os.makedirs(config.SINGBOX_OUTPUT_DIR, exist_ok=True)
...
def main():
    ...
    try:
        ...
        # **تغییر یافته**: فراخوانی run_full_pipeline برای مدیریت تمامی فازها
